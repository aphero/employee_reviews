...
# considered long.
def has_even?(array)
  array.any? { |x| x % 2 == 0 }  # Can also use .even? instead of modulo
end
...
def just_short(array)
  array.select { |x| x.length <= 4 }
end

def no_long(array)
  array.reject { |x| x.length > 4 }
end

def product(array)
  array.reduce { |nickel, pickle|  nickel * pickle } # The nickel is the accumulator, the pickle is the current item being indexed.
end
